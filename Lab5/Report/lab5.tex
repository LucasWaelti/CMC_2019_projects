...
Figure ? shows the different behaviour of the isometric muscle contraction with increasing length. Until approximately 0.11m the muscle is in a fully active state, where the force is only generated in the contractile element. The maximal force of the contractile element is produced when its length $l_{CE}$ is equal to the muscles optimal fiber length $l_{opt}$. After the contractile element reached its peak force it starts decreasing while the passive force starts increasing. The lengthening of the muscle leads to high forces due to the passive elements while the active force becomes zero. 
%The total force (i.e. tendon force) has a local maximum when the active force is at its peak followed by a small decrease while the passive force starts rising, generating relatively high total forces.
...
The stimulation only influences the contractile element and hence the passive force remains the same, which dominate the total force at longer lengths. The higher the stimulation the larger the active force generated by the contractile element. In figure ?, you can see that with increasing stimulation the peak of the active force gets bigger. Important to notice is also that the decrease after the local peak gets smaller with smaller activation since the raising passive force can better compensate the decreasing active force.
...
In figure ?, the force-length curves of the different fiber lengths ($l_{opt}$) are plotted. As expected, the fiber length influences the peak of active force. The peak is where the length of the contractile element is equal to the fiber length, as previously already mentionend. For shorter fiber lengths the active force maximum is reached earlier and vice versa. Also a change in fiber length causes a horizontal shift in the passiv force-length relation.
...
Because as already seen in the force-length plots, passive forces help the muscle at longer lengths and preventing it from over stretching.
...
As one can see in figure ?, the lower the stimulation the lower is the load under which the muscle still can contract. Additionally, since the force generated by the muscle is lower with lower stimulation the maximum velocity decreases more rapidly because this force can only bear smaller external loads.
